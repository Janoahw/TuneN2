...
export default function ModerationPage() {
  ...
  const columns = [
    ...
    {
      header: 'Content',
      accessor: (row: any) => row.song?.title || 'Content unavailable',
    },
    {
      header: 'Reporter',
      accessor: (row: any) => row.reporter?.displayName || row.reporter?.email || 'Unknown',
    },
    {
      header: 'Status',
      accessor: (row: any) => (
    ...
    <Layout>
      <div className="max-w-7xl">
        <div className="mb-8">
          ...
          <p className="mt-2 text-sm text-[#8E8E93]">Review and manage reported content</p>
        </div>
        ...
        <div className="mb-6">
          <select
            ...
            className="h-10 rounded-lg border border-[#1A1A1E] bg-[#111114] px-4 py-2 text-sm text-white outline-none focus:ring-2 focus:ring-[#00CCCC]"
          >
